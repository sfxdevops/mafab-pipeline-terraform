...
resource "aws_codebuild_project" "mfb_codebuild_project" {
  depends_on  = [aws_ecr_repository.platform-field-validator-ecr]
  ...
  source_version = (var.environment == "prod" ? "master" : var.environment == "staging" ? "DEVOPSS-226" : "staging-dev")
...
data "template_file" "buildspec" {
  ...
  vars = {
    ecr_repo_url = aws_ecr_repository.platform-field-validator-ecr.repository_url
    region       = var.region
