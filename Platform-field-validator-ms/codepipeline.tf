  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        BranchName           = (var.environment == "prod" ? "master" : var.environment == "staging" ? "DEVOPSS-226" : "staging-dev")
...
resource "aws_codedeploy_app" "mfb_codedeploy_app" {
  name = "mfb-platform-field-validator-ms-app"
...
resource "aws_codedeploy_deployment_group" "mfb_codedeploy_group" {
  ...
  deployment_group_name = "mfb-platform-field-validator-ms-group"
