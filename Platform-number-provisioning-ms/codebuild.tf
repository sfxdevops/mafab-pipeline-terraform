...
resource "aws_codebuild_project" "mfb_codebuild_project" {
  depends_on    = [aws_ecr_repository.platform-number-provisioning-ecr]
  ...
  source_version = (var.environment == "prod" ? "master" : var.environment == "staging" ? "DEVOPS-1728" : "staging-dev")
...
data "template_file" "buildspec" {
  ...
  vars = {
    ecr_repo_url = aws_ecr_repository.platform-number-provisioning-ecr.repository_url
    region       = var.region
