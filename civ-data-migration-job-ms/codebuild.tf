...
# resource "aws_iam_role_policy" "civ-role-policy" {
...
resource "aws_codebuild_project" "civ_codebuild_project" {
  depends_on    = [aws_ecr_repository.civ-data-migration-job-ms-ecr]
...
data "template_file" "buildspec" {
  ...
  vars = {
    ecr_repo_url = aws_ecr_repository.civ-data-migration-job-ms-ecr.repository_url
    region       = var.region
