...
resource "aws_codebuild_project" "mfb_codebuild_project" {
  depends_on    = [aws_ecr_repository.mfb-biodata-hlr-push-retrial-job-ms-ecr]
...
data "template_file" "buildspec" {
  ...
  vars = {
    ecr_repo_url = aws_ecr_repository.mfb-biodata-hlr-push-retrial-job-ms-ecr.repository_url
    region       = var.region
