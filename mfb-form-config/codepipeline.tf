  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        BranchName           = (var.environment == "prod" ? "master" : var.environment == "staging" ? "snapshot" : "DEVOPS-1728")
...
resource "aws_codedeploy_deployment_group" "mfb_codedeploy_group" {
  ...
  deployment_group_name = "mfb-form-config-group"
