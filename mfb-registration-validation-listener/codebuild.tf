...
resource "aws_codebuild_project" "mfb_codebuild_project" {
  depends_on    = [aws_ecr_repository.mfb-registration-validation-listener-ecr]
  ...
  source_version = (var.environment == "prod" ? "master" : var.environment == "staging" ? "DEVOPSS-188" : "staging-dev")
...
data "template_file" "buildspec" {
  ...
  vars = {
    ecr_repo_url = aws_ecr_repository.mfb-registration-validation-listener-ecr.repository_url
    region       = var.region
