  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        BranchName           = (var.environment == "prod" ? "master" : var.environment == "staging" ? "snapshot-dev" : "DEVOPSS-276")
...
resource "aws_codedeploy_app" "mfb_codedeploy_app" {
  name = "mfb-sim-activation-job-app"
...
resource "aws_codedeploy_deployment_group" "mfb_codedeploy_group" {
  ...
  deployment_group_name = "mfb-sim-activation-job-group"
